...
class ClassificationConformalPredictor(BaseConformalPredictor):
    ...
    def fit(self, y_true, y_pred,probs_calibration, alpha):
        ...
        softmax_probabilities = torch.tensor(y_pred, dtype=torch.float32)
        true_labels = torch.tensor(y_true, dtype=torch.long)
    ...
    def get_conformal_results(self, y_true, y_pred,probs_test, threshold):
        ...
        """
        prediction_sets = self.predict(y_pred, threshold)
        
        # Calculate coverage
        coverage = self.evaluate_coverage(prediction_sets, y_true)
