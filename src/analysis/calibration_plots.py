...
def dataset_figure_dir(dataset: str) -> Path:
    """Return (and create) the output directory for a given dataset."""
    path = FIGURE_DIR / dataset
...
MODE_STYLE_OVERRIDES: Dict[str, Dict[str, object]] = {
    "global": {"color": COLOR_PALETTE[0], "linestyle": "-"},
    "hybrid": {"color": COLOR_PALETTE[1], "linestyle": "--"},
    "hybrid tau5": {"color": COLOR_PALETTE[1], "linestyle": "--"},
    "mondrian": {"color": COLOR_PALETTE[2], "linestyle": "-."},
    "mondrian tau5": {"color": COLOR_PALETTE[2], "linestyle": "-."},
}
...
def get_mode_style(mode: str, index: int) -> Dict[str, object]:
    ...
    linestyle = _LINE_STYLE_CYCLE[index % len(_LINE_STYLE_CYCLE)]
...
def plot_coverage_curves(df: pd.DataFrame,
    ...
    fig.tight_layout()
    dataset_dir = dataset_figure_dir(dataset)
    out_path = dataset_dir / f"coverage_{dataset}_{model}.pdf"
...
def plot_interval_sizes(df: pd.DataFrame,
    ...
    fig.tight_layout()
    dataset_dir = dataset_figure_dir(dataset)
    out_path = dataset_dir / f"psize_{dataset}_{model}.pdf"
    fig.savefig(out_path)
    fig.savefig(out_path.with_suffix(".png"))
    plt.close(fig)
    return out_path
...
def plot_repeat_metrics(df: pd.DataFrame,
    ...
    fig.tight_layout()
    dataset_dir = dataset_figure_dir(dataset)
    out_path = dataset_dir / f"{metric}_{dataset}_{model}.pdf"
    fig.savefig(out_path)
    fig.savefig(out_path.with_suffix(".png"))
    plt.close(fig)
    return out_path
...
def main() -> None:
    ...
    if args.modes:
        ...
        if not modes:
            print(
                f"Skipping dataset={args.dataset}, model={args.model}: no conformal modes found."
            )
            return
    ...
    if alpha_summary[
        ...
        print(f"Skipping dataset={args.dataset}, model={args.model}: no alpha summary rows found.")
        return
    ...
    if repeat_summary[
        ...
        print(f"Warning: no repeat summary rows for dataset={args.dataset}, model={args.model}.")
